...
namespace DicomDB.BL.Services
{
    public class CityService : BaseService
    {
        ...
        public static CityResponse GetLastCity(CityRequest request)
        {
            ...
            RunBase(
                ...
                (CityRequest req) =>
                {
                    ...
                    {
                        var query = request
                            .context.City.Select(p => new CityDTO
                            {
                                Id = p.Id,
                                CityName = p.CityName,
                                CityNameAr = p.CityNameAr,
        ...
        public static CityResponse DeleteCity(CityRequest request)
        {
            ...
            RunBase(
                ...
                (CityRequest req) =>
                {
                    try
                    {
                        var City = request.context.City.FirstOrDefault(c =>
                            c.Id == request.CityDTO.Id
                        );
                        if (City != null)
                        {
                            //update IsDeleted
        ...
        public static IQueryable<CityDTO> ApplyFilter(IQueryable<CityDTO> query, CityDTO cityRecord)
        {
            ...
            {
                query = query.Where(c =>
                    (!string.IsNullOrEmpty(c.CityName) && c.CityName.Contains(cityRecord.Search))
                    || (
                        !string.IsNullOrEmpty(c.CityNameAr)
                        && c.CityNameAr.Contains(cityRecord.Search)
                    )
                );
            }
