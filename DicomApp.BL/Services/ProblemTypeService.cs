    ...
    public class ProblemTypeService : BaseService
    {
        public static ProblemTypeResponse ListProblemType(ProblemTypeRequest request)
        {
            ...
            RunBase(
                request,
                response,
                (ProblemTypeRequest req) =>
                {
                    try
                    {
                        var query = request
                            .context.ProblemType.Where(p =>
                                (p.IsDeleted.HasValue ? !p.IsDeleted.Value : true)
                            )
                            .Select(p => new ProblemTypeDTO
                            {
                        ...
                        query = OrderByDynamic(
                            query,
                            request.OrderByColumn ?? "ProblemTypeId",
                            request.IsDesc
                        );
            ...
            RunBase(
                ...
                (ProblemTypeRequest req) =>
                {
                    try
                    {
                        ...
                        var ProblemType = request.context.ProblemType.FirstOrDefault(c =>
                            c.ProblemTypeId == model.ProblemTypeId
                        );
                        if (ProblemType != null)
                        {
                            ...
                            response.Message = SystemEnums.DeletedSuccessfully.ToString();
                            response.Success = true;
                            response.StatusCode = HttpStatusCode.OK;
                        }
                        else
                        {
                            response.Message = SystemEnums.InvalidData.ToString();
                            response.Success = false;
                        }
                    }
                    catch (Exception ex)
        ...
        private static ProblemType AddOrEditProblemType(
            ProblemTypeDTO ProblemTypeRecord,
        ...
        private static IQueryable<ProblemTypeDTO> ApplyFilter(
            IQueryable<ProblemTypeDTO> query,
            ProblemTypeDTO record
        )
        {
            if (!string.IsNullOrEmpty(record.Search))
            {
                query = query.Where(c =>
                    ...
                    && (c.NameEn.Contains(record.Search) || c.NameAr.Contains(record.Search))
                );
            }
            return query;
        }
