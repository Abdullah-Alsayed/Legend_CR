...
namespace DicomApp.BL.Services
{
    public class ShipmentService : BaseService
    {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        ship.ShipmentTypeId = request.ShipDTO.SettingDTO.IsStock
                            ? (int)EnumShipmentType.StockDelivery
                            : (int)EnumShipmentType.PickupDelivery;
                        ship.ShipmentServiceId = request.ShipDTO.ShipmentServiceId;
                        ship.StatusId = request.ShipDTO.SettingDTO.IsStock
                        ...
                        CommonUser user = request.context.CommonUser.FirstOrDefault(u =>
                            u.Id == request.ShipDTO.VendorDetailsDTO.VendorId
                        );
                        ship.VendorName = user.Name;
                        ship.VendorPhone = user.PhoneNumber;

                        // Check Game fees
                        if (request.ShipDTO.SettingDTO.GameId.HasValue)
                        {
                            Game pack = request
                                .context.Game.Include(p => p.Category)
                                .FirstOrDefault(u => u.Id == request.ShipDTO.SettingDTO.GameId);
                            ship.GameId = pack.Id;
                            ship.WarehouseGameId = pack.Id;
                        }
                        ...
                        // Check weight fees
                        ship.WeightFees = BaseHelper.CalcWeightFees(
                            request.ShipDTO.SettingDTO.Weight
                        );
                        ship.VendorCash -= BaseHelper.CalcWeightFees(
                            request.ShipDTO.SettingDTO.Weight
                        );

                        // Check shipping fees
                        ship.ShippingFees = request
                            .context.ZoneTax.FirstOrDefault(z => z.ZoneId == ship.ZoneId)
                            .Tax;
                        ...
                        if (ship.IsStock)
                        {
                            var selectedStockIDs = request
                                .ShipDTO.ProductIDs.Split(',')
                                .Select(int.Parse)
                                .ToList();
                            var selectedStockQuantities = request
                                .ShipDTO.ProductsQuantity.Split(',')
                                .Select(int.Parse)
                                .ToList();
                            var selectedStockPrices = request
                                .ShipDTO.ProductsPrice.Split(',')
                                .Select(int.Parse)
                                .ToList();
                            ...
                            for (int i = 0; i < selectedStockIDs.Count(); i++)
                            {
                                ...
                                {
                                    var product = request.context.VendorProduct.FirstOrDefault(z =>
                                        z.VendorProductId == selectedStockIDs[i]
                                    );
                                    request.context.ShipmentItem.Add(
                                        new ShipmentItem()
                                        {
                        ...
                        if (!ship.IsStock && ship.IsPartialDelivery)
                        {
                            ...
                            {
                                var items = JsonConvert.DeserializeObject<List<ShipmentItem>>(
                                    request.ShipDTO.PartialItems
                                );
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        ship.ShippingFees = request
                            ...
                            .Tax;
                        ship.Total =
                            request.ShipDTO.RefundCash.Value
                            + ship.ShippingFees
                            + ship.RefundFees.Value;
        ...
        public static ShipmentResponse EditShipment(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        var ship = request.context.Shipment.FirstOrDefault(s =>
                            s.ShipmentId == request.ShipDTO.ShipmentId
                        );
                        ...
                        // Check Game >> NULL then NOT NULL
                        if (
                            !ship.WarehouseGameId.HasValue
                            && request.ShipDTO.SettingDTO.WarehouseGameId.HasValue
                        )
                        {
                            Game pack = request.context.Game.FirstOrDefault(u =>
                                u.Id == request.ShipDTO.SettingDTO.GameId
                            );
                            ship.GameId = pack.Id;
                            ship.WarehouseGameId = pack.Id;
                        } // Check Game >> NOT NULL then NOT NULL
                        else if (
                            ship.WarehouseGameId.HasValue
                            && request.ShipDTO.SettingDTO.WarehouseGameId.HasValue
                        )
                        {
                            // Update Old Data
                            Game pack = request.context.Game.FirstOrDefault(u =>
                                u.Id == ship.WarehouseGameId
                            );

                            request.context.SaveChanges();

                            // Insert New Data
                            pack = request.context.Game.FirstOrDefault(u =>
                                u.Id == request.ShipDTO.SettingDTO.WarehouseGameId
                            );
                        ...
                        var oldItems = request.context.ShipmentItem.Where(i =>
                            i.ShipmentId == ship.ShipmentId
                        );
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        ship.CallNotAnswerCount = request
                            .ShipDTO
                            .CustomerFollowUpDTO
                            .CallNotAnswerCount;
        ...
        public static ShipmentResponse EditGame(ShipmentRequest request)
        {
            var response = new ShipmentResponse();
            RunBase(
                request,
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        ship.WeightFees = BaseHelper.CalcWeightFees(
                            request.ShipDTO.SettingDTO.WarehouseWeight
                        );
                        ship.VendorCash -= ship.WeightFees;
                        ...
                        response.Message = "Shipment Game" + ship.RefId + " successfully updated";
                        response.Success = true;
                        response.StatusCode = HttpStatusCode.OK;
                    }
                    catch (Exception ex)
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        // Check weight fees
                        result.WeightFees = BaseHelper.CalcWeightFees(
                            request.ShippingCalculatorDTO.Weight
                        );
                        ...
                        result.DestinationZoneId = request
                            .context.City.FirstOrDefault(z =>
                                z.Id == request.ShippingCalculatorDTO.DestinationAreaId
                            )
                            .ZoneId.Value;
                        result.DestinationShippingFees = request
                            .context.ZoneTax.FirstOrDefault(z =>
                                z.ZoneId == result.DestinationZoneId
                            )
                            .Tax;

                        result.TotalFees =
                            result.WeightFees
                            + result.SourceShippingFees
                            + result.DestinationShippingFees;
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        if (request.ShipDTO.ShipmentIDs != null)
                            shipments = request.context.Shipment.Where(r =>
                                request.ShipDTO.ShipmentIDs.Contains(r.ShipmentId)
                            );
                        else
                            shipments = request.context.Shipment.Where(r =>
                                r.ShipmentId == request.ShipDTO.ShipmentId
                            );

                        foreach (var ship in shipments)
                        {
                            var shipItems = request.context.ShipmentItem.Where(i =>
                                i.ShipmentId == ship.ShipmentId
                            );
                            foreach (var item in shipItems)
                            {
                                ...
                                if (
                                    request.ShipDTO.StatusId == (int)EnumStatus.Ready_For_Delivery
                                    && ship.IsStock
                                )
                                {
                                    // Update available stock
                                    var product = request.context.VendorProduct.FirstOrDefault(p =>
                                        p.VendorProductId == item.VendorProductId
                                    );
                                    ...
                                    request.ShipDTO.StatusId == (int)EnumStatus.At_Warehouse
                                    && ship.IsStock
                                )
                                {
                                    // Update available stock
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        request.context.ShipmentProblem.Add(
                            new ShipmentProblem
                            {
                                CreatedAt = DateTime.Now,
                                CreatedBy = request.UserID,
        ...
        public static ShipmentResponse Deliver(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        var shipments = request.context.Shipment.Where(r =>
                            request.ShipDTO.ShipmentIDs.Contains(r.ShipmentId)
                        );
                        ...
                        foreach (var ship in shipments)
                        {
                            ...
                            if (request.ShipDTO.ShipItemDTOs != null)
                            {
                                ...
                                {
                                    var shipItem = request.context.ShipmentItem.FirstOrDefault(i =>
                            ...
                            {
                                var lstShipItems = request.context.ShipmentItem.Where(i =>
                                    i.ShipmentId == ship.ShipmentId
                                );
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        ship.LastModifiedBy = request.UserID;
                        ship.StatusId =
                            request.ShipDTO.CustomerFollowUpDTO.NextCallOn.Value.Day
                            == DateTime.Now.Day
                            ...
                            var follow = new FollowUpDTO
                            {
                                ...
                                Comment = request
                                    .context.CommonUser.FirstOrDefault(u =>
                                        u.Id == request.ShipDTO.DeliveryManId
                                    )
                                    .Name,
                                CreatedBy = request.UserID,
                                ShipmentId = ship.ShipmentId,
        ...
        public static ShipmentResponse Unassigned(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        var shipment = request.context.Shipment.FirstOrDefault(s =>
                            request.ShipDTO.ShipmentId == s.ShipmentId
                        );
                        ...
                        var shipItems = request.context.ShipmentItem.Where(i =>
                            i.ShipmentId == shipment.ShipmentId
                        );
                        foreach (var item in shipItems)
                        {
                            if (item.StatusId != (int)EnumStatus.Delivered)
                        ...
                        var follow = new FollowUpDTO
                        {
                            FollowUpTypeId = (int)EnumFollowupType.Order_Updated,
                            Title = request
                                .context.Status.FirstOrDefault(s => s.Id == shipment.StatusId)
                                .NameEN,
        ...
        public static ShipmentResponse ReceiveReturns(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        var ships = request.context.Shipment.Where(r =>
                            request.ShipDTO.ShipmentIDs.Contains(r.ShipmentId)
                        );
                        foreach (var ship in ships)
                        {
                            ...
                            {
                                var oldShip = request.context.Shipment.FirstOrDefault(f =>
                                    f.ShipmentId == ship.ShipToRefundId
                                );
                                ...
                                var oldShipItems = request.context.ShipmentItem.Where(i =>
                                    i.ShipmentId == oldShip.ShipmentId
                                );
        ...
        public static ShipmentResponse ReceiveCash(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    ...
                    {
                        var ships = request.context.Shipment.Where(r =>
                            r.DeliveryManId == request.ShipDTO.DeliveryManId
                        ...
                        foreach (var ship in ships)
                        {
                            ...
                            var CourierAccountId = request
                                .context.Account.FirstOrDefault(a =>
                                    a.UserId == request.ShipDTO.DeliveryManId
                                )
                                .AccountId;
                            var VendorAccountId = request
                                .context.Account.FirstOrDefault(a => a.UserId == ship.VendorId)
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        ship.LastModifiedBy = request.UserID;
                        ship.ShippingFeesPaid =
                            request.ShipDTO.FeesDetailsDTO != null
                                ? request.ShipDTO.FeesDetailsDTO.ShippingFeesPaid
                                : 0;

                        var problemDTO = request.ShipDTO.ProblemDTOs.First();

                        // Delete last existing problem
                        var problem = request.context.ShipmentProblem.LastOrDefault(s =>
                            s.IsCourierProblem == problemDTO.IsCourierProblem
                            && s.ShipmentId == ship.ShipmentId
                            && !s.IsDeleted
                        );
                        if (problem != null)
                        ...
                        var follow = new FollowUpDTO
                        {
                            ...
                            Title = request
                                .context.ProblemType.FirstOrDefault(s =>
                                    s.ProblemTypeId == shipProblem.ProblemTypeId
                                )
                                .NameEn,
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        var follow = new FollowUpDTO
                        {
                            ...
                            Title = request
                                .context.ProblemType.FirstOrDefault(s =>
                                    s.ProblemTypeId == problemDTO.ProblemTypeId
                                )
                                .NameEn,
                            Comment = problem.Note,
                            CreatedBy = request.UserID,
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        var shipProblem = request.context.ShipmentProblem.LastOrDefault(s =>
                            s.IsCourierProblem == problemDTO.IsCourierProblem
                            && s.ShipmentId == ship.ShipmentId
                            && !s.IsSolved
                            && !s.IsDeleted
                        ...
                        {
                            FollowUpTypeId =
                                problemDTO.IsCourierProblem && !problemDTO.IsSolved
                                    ? (int)EnumFollowupType.Replied_To_Courier
                                    : (int)EnumFollowupType.Problem_Solved,
                            Title =
                                problemDTO.IsCourierProblem && !problemDTO.IsSolved
                                    ? "Replied To Courier"
                                    : "Problem Solved",
                ...
                (ShipmentRequest req) =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(request.ShipDTO.RefId))
                            ship = request.context.Shipment.Where(s =>
                                s.ShipmentId == request.ShipDTO.ShipmentId && !s.IsDeleted
                            );
                        else
                            ship = request.context.Shipment.Where(s =>
                                s.RefId.Contains(request.ShipDTO.RefId)
                            );

                        var query = ship.Select(s => new ShipDTO
                        {
                            ...
                            ShipToRefundRefId = s.ShipToRefundId.HasValue
                                ? request
                                    .context.Shipment.FirstOrDefault(rf =>
                                        rf.ShipmentId == s.ShipToRefundId.Value
                                    )
                                    .RefId
                                : null,
                            HasRefunded = request.context.Shipment.Any(rr =>
                                rr.ShipToRefundId == s.ShipmentId
                            ),
                            ...
                            StatusDTO = new StatusDTO
                            {
                                ...
                                DeliveryManName = s.DeliveryManId.HasValue
                                    ? s.DeliveryMan.Name
                                    : null,
                            },

                            CustomerDetailsDTO = request.HasCustomerDetailsDTO
                            ...
                            VendorDetailsDTO = request.HasVendorDetailsDTO
                            ...
                            SettingDTO = request.HasSettingDTO
                                ? new ShipSettingDTO
                                {
                                    ...
                                    GameId = s.GameId,
                                    CategoryId = s.GameId.HasValue ? s.Game.CategoryId : 0,
                                    WarehouseGameId = s.WarehouseGameId,
                                    WarehouseCategoryId = s.WarehouseGameId.HasValue
                                        ? s.WarehouseGame.CategoryId
                                        : 0,
                                    GameName = s.WarehouseGameId.HasValue
                                        ? s.WarehouseGame.NameEn
                                        : s.GameId.HasValue
                                            ? s.Game.NameEn
                                            : null,
                                    CategoryName = s.WarehouseGameId.HasValue
                                        ? s.WarehouseGame.Category.NameEn
                                        : s.GameId.HasValue
                                            ? s.Game.Category.NameEn
                                            : null,
                                }
                                : null,

                            FeesDetailsDTO = request.HasFeesDetailsDTO
                                ? new FeesDetailsDTO
                                {
                                    IsAfford = s.IsAfford,
                            ...
                            CustomerFollowUpDTO = request.HasCustomerFollowUpDTO
                                ? new CustomerFollowUpDTO
                                {
                            ...
                            ProblemDTOs = request.HasProblemDTOs
                                ? s
                                    .ShipmentProblem.Where(sh => !sh.IsDeleted)
                            ...
                            FollowUpDTOs = request.HasFollowUpDTOs
                                ? s.FollowUp.Select(f => new FollowUpDTO
                                {
                            ...
                            ShipItemDTOs = request.HasShipItemDTOs
                                ? s.ShipmentItem.Select(i => new ShipItemDTO
                                {
                                    ...
                                    Quantity = i.Quantity,
                                    Price = i.Price,
                                    Name = i.VendorProductId.HasValue
                                        ? i.VendorProduct.Name
                                        : i.Name,
                                    Size = i.VendorProductId.HasValue
                                        ? i.VendorProduct.Size
                                        : i.Size,
                                    ImageUrl = i.ImageUrl,
                                    StatusId = i.StatusId,
        ...
        public static ShipmentResponse GetAllShipments(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    ...
                    {
                        var query = request
                            .context.Shipment.Where(s => !s.IsDeleted)
        ...
        public static ShipmentResponse GetVendorShipments(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    ...
                    {
                        var query = request
                            .context.Shipment.Where(s =>
                                !s.IsDeleted
                                && (
                                    request.ShipDTO.VendorDetailsDTO.VendorId > 0
                                        ? (
                                            s.VendorId == request.ShipDTO.VendorDetailsDTO.VendorId
                                            && GetStatusGroup(request.ShipDTO.StatusId)
                                                .Contains(s.StatusId)
                                        )
                                        : true
                                )
                            ...
                            .Select(s => new VendorReportDTO
                            {
                                ...
                                CurrentShipsCount = s.Where(g =>
                                        GetStatusGroup((int)EnumStatus.Current).Contains(g.StatusId)
                                    )
                                    .Count(),
                                DoneShipsCount = s.Where(g =>
                                        GetStatusGroup((int)EnumStatus.Done).Contains(g.StatusId)
                                    )
                                    .Count(),
                                CancelledShipsCount = s.Where(g =>
                                        GetStatusGroup((int)EnumStatus.Cancelled)
                                            .Contains(g.StatusId)
                                    )
                                    .Count(),
                                ...
                                SuccessPerc = (int)(
                                    (
                                        (double)
                                            s.Where(g =>
                                                    GetStatusGroup((int)EnumStatus.Done)
                                                        .Contains(g.StatusId)
                                                )
                                ...
                                TotalRefunded = request
                                    .context.AccountTransaction.Where(t =>
                                        t.ReceiverId == t.VendorId
                                        && t.VendorId
                                            == request
                                                .context.Account.FirstOrDefault(f =>
                                                    f.UserId == s.Key
                                ...
                                Balance = request
                                    .context.Account.FirstOrDefault(t => t.UserId == s.Key)
                                    .Balance,

                                lstShips = s.OrderByDescending(t => t.ShipmentId)
                                    .Select(t => new ShipDTO
                                    {
                                        ...
                                        StatusDTO = new StatusDTO
                                        {
                                            ...
                                            DeliveryManName = t.DeliveryManId.HasValue
                                                ? t.DeliveryMan.Name
                                                : null,
                                        },
                                        ...
                                        TransactionDTOs = t.AccountTransaction.Select(
                                            tr => new AccountTransactionDTO
                                            {
                                                ...
                                                StatusName = tr.Status.NameEN,
                                                StatusDTO = new StatusDTO
                                                {
                                                    Id = tr.StatusId.Value,
                                                    Name = tr.Status.NameEN,
                                                },
        ...
        public static ShipmentResponse GetShipmentItems(ShipmentRequest request)
        {
            ...
            RunBase(
                ...
                (ShipmentRequest req) =>
                {
                    ...
                    {
                        var query = request
                            .context.ShipmentItem.Where(s =>
                                s.ShipmentId == request.ShipItemDTO.ShipmentId && !s.IsDeleted
                            )
                            .Select(p => new ShipItemDTO
                            {
                                ...
                                {
                                    Id = p.StatusId,
                                    Name = p.Status.NameEN,
                                    DeliveryManName = p.Shipment.DeliveryManId.HasValue
                                        ? p.Shipment.DeliveryMan.Name
                                        : null,
                                },
                            });
        ...
        private static IQueryable<ShipDTO> ApplyFilter(
            IQueryable<ShipDTO> query,
            ShipDTO filter,
            int RoleId,
            int UserId
        )
        {
            ...
            if (filter.VendorDetailsDTO != null)
                if (filter.VendorDetailsDTO.VendorId > 0)
                    query = query.Where(c =>
                        c.VendorDetailsDTO.VendorId == filter.VendorDetailsDTO.VendorId
                    );
                ...
                query = query.Where(s =>
                    (
                        ...
                        || (
                            ...
                            && s.ShipItemDTOs.Where(i => i.StatusId != (int)EnumStatus.Delivered)
                                .Count() > 0
                        )
                        || (s.StatusId == (int)EnumStatus.Delivered)
                            && s.ShipItemDTOs.Where(i =>
                                (i.StatusId != (int)EnumStatus.Delivered)
                                || (
                                    i.StatusId == (int)EnumStatus.Delivered
                                    && i.CourierQuantityDelivered < i.Quantity
                                )
                            )
                                .Count() > 0
                    )
