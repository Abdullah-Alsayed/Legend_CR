...
namespace DicomApp.Portal.ViewComponents
{
    public class RoleVComponent : ViewComponent
    {
        ...
        public RoleVComponent(LegendDBContext context)
        {
            _context = context;
        }
        ...
        )
        {
            ...
            if (roleResponse.RoleDTOs != null)
            {
                ViewBag.noOfPages = Math.Ceiling(
                    (double)roleResponse.TotalCount / roleRequest.PageSize
                );
                ViewBag.hasMore =
                    roleResponse.TotalCount
                    > (
                        roleResponse.RoleDTOs.Count + (roleRequest.PageIndex * roleRequest.PageSize)
                    );
