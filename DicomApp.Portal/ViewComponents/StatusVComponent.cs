...
namespace DicomApp.Portal.ViewComponents
{
    public class StatusVComponent : ViewComponent
    {
        ...
        public StatusVComponent(LegendDBContext context)
        {
            _context = context;
        }
        ...
        )
        {
            ...
            {
                ViewBag.noOfPages = Math.Ceiling(
                    (double)statusResponse.TotalCount / statusRequest.PageSize
                );
                ViewBag.hasMore =
                    statusResponse.TotalCount
                    > (
                        statusResponse.StatusDTOs.Count
                        + (statusRequest.PageIndex * statusRequest.PageSize)
                    );
