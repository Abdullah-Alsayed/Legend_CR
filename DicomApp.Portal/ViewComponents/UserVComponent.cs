...
namespace DicomApp.Portal.ViewComponents
{
    public class UserVComponent : ViewComponent
    {
        ...
        public UserVComponent(LegendDBContext context)
        {
            _context = context;
        }
        ...
        )
        {
            ...
            if (userResponse.UserDTOs != null)
            {
                ViewBag.noOfPages = Math.Ceiling(
                    (double)userResponse.TotalCount / userRequest.PageSize
                );
                ViewBag.hasMore =
                    userResponse.TotalCount
                    > (
                        userResponse.UserDTOs.Count + (userRequest.PageIndex * userRequest.PageSize)
                    );
