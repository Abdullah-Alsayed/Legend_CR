...
//            RunBase(request, response, (AccountRequest req) =>
//            {
//                try
//                {
...
//                try
//                {
//                    var account = request.context.Account.FirstOrDefault(s => s.AccountId == request.AccountDTO.AccountId);
...
//                try
//                {
//                    var query = request.context.Account.Where(s => (s.AccountId == request.AccountDTO.AccountId || s.UserId == request.AccountDTO.UserId) && !s.IsDeleted)
...
//                                                 s.AccountTransactionReceiver.Select(t => new AccountTransactionDTO
//                                                 {
...
//                                                     VendorId = t.VendorId,
//                                                     StatusId = t.StatusId,
//                                                     StatusName = t.StatusId.HasValue ? t.Status.Name : null,
...
//                try
//                {
//                    var query = request.context.Account.Where(s => !s.IsDeleted)
//                    .Select(s => new AccountDTO
//                    {
//                        AccountId = s.AccountId,
//                        Name = s.Name,
...
//                        TransactionDTOs = request.HasTransactionsDTOs ?
//                                                 s.AccountTransactionVendor.Select(t => new AccountTransactionDTO
//                                                 {
//                                                     AccountTransactionId = t.AccountTransactionId,
//                                                     RefId = t.RefId,
//                                                     ShipRefId = t.ShipmentId.HasValue ? t.Shipment.RefId : null,
...
//                        query = ApplyFilter(query, request.AccountDTO, request.RoleID, request.UserID);
...
//                try
//                {
//                    var hasDelvieredShipsIDs = request.context.FollowUp.Where(f => f.StatusId == (int)EnumStatus.Delivered).Select(f => f.ShipmentId.Value).ToList();

//                    var nonPaidShips = request.context.Shipment.Where(s => s.VendorId == request.AccountDTO.UserId
//                                                                        && !s.IsPaidToVendor
//                                                                        && hasDelvieredShipsIDs.Contains(s.ShipmentId))
//                    .Select(t => t.ShipmentId).ToList();

//                    var query = request.context.Account.Where(s => !s.IsDeleted && s.RoleId == (int)EnumRole.Vendor && s.UserId == request.AccountDTO.UserId)
...
//                        Total = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == s.AccountId && nonPaidShips.Contains(t.ShipmentId.Value))
//                        .Sum(t => t.Total),

//                        TransactionDTOs = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == s.AccountId && nonPaidShips.Contains(t.ShipmentId.Value))
...
//                try
//                {
//                    var query = request.context.Account.Where(s => !s.IsDeleted && s.RoleId == (int)EnumRole.Vendor)
//                    .Select(s => new TreasuryDTO
//                    {
//                        AccountId = s.AccountId,
//                        Balance = s.Balance,
...
//                        TotalDelivered = s.AccountTransactionVendor
...
//                        TotalRefunded = s.AccountTransactionVendor
//                        .Where(t => (t.ReceiverId == s.AccountId)
//                                  && t.StatusId == (int)EnumStatus.Refunded && t.TypeId == (int)EnumTransactionType.Withdraw)
//                        .Sum(t => t.Total),

//                        TotalOthers = s.AccountTransactionVendor
...
//                        ShippingFees = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit)
//                        .Sum(t => t.ShippingFees),

//                        PackingFees = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit)
//                        .Sum(t => t.PackingFees),

//                        WeightFees = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit)
//                        .Sum(t => t.WeightFees),

//                        SizeFees = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit)
//                        .Sum(t => t.SizeFees),

//                        PartialDeliveryFees = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit)
//                        .Sum(t => t.PartialDeliveryFees),

//                        PickupFees = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit)
//                        .Sum(t => t.PickupFees),

//                        CancelFees = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == s.AccountId && t.TypeId == (int)EnumTransactionType.Withdraw)
//                        .Sum(t => t.CancelFees),

//                        Withdraw = s.AccountTransactionVendor
//                        .Where(t => t.ReceiverId == s.AccountId && t.TypeId == (int)EnumTransactionType.Withdraw)
//                        .Sum(t => t.Total),

//                        TransactionDTOs = s.AccountTransactionVendor
//                        .Where(t => (t.ReceiverId == s.AccountId || t.ReceiverId == (int)EnumAccountId.RED_Main_Account))
//                        .Select(t => new AccountTransactionDTO
//                        {
//                            AccountTransactionId = t.AccountTransactionId,
//                            ShipRefId = t.ShipmentId.HasValue ? t.Shipment.RefId : null,
...
//                            StatusId = t.StatusId,
//                            StatusName = t.StatusId.HasValue ? t.Status.Name : null,
//                            StatusDTO = t.StatusId.HasValue ? new StatusDTO
//                            {
//                                Id = t.StatusId.Value,
//                                Name = t.Status.Name,
//                            } : null,
...
//        private static IQueryable<AccountDTO> ApplyFilter(IQueryable<AccountDTO> query, AccountDTO filter, int RoleId, int UserId)
//        {
