...
//            RunBase(request, response, (CashTransferRequest req) =>
//            {
//                try
//                {
//                    var VendorAccount = request.context.Account.FirstOrDefault(a => a.UserId == request.CashTransferDTO.VendorId);
//                    var MainTreasuryAccount = request.context.Account.FirstOrDefault(a => a.AccountId == (int)EnumAccountId.Main_Treasury);
//                    var selectedTransIDs = request.CashTransferDTO.TransIDs.Split(',').Select(int.Parse).ToList();
//                    var Transactions = request.context.AccountTransaction.Where(r => selectedTransIDs.Contains(r.AccountTransactionId)).ToList();
...
//                    cashTransfer.RefId = BaseHelper.GenerateRefId(EnumRefIdType.Cash_Transfer, cashTransfer.CashTransferId);
...
//                        {
//                            var ship = request.context.Shipment.FirstOrDefault(r => r.ShipmentId == trans.ShipmentId);
...
//                    response.Message = "New Cash Transfer #" + cashTransfer.RefId + " successfully added";
...
//                try
//                {
//                    var cashTransfer = request.context.CashTransfer.FirstOrDefault(s => s.CashTransferId == request.CashTransferDTO.CashTransferId);
...
//                try
//                {
//                    var query = request.context.CashTransfer.Where(s => s.CashTransferId == request.CashTransferDTO.CashTransferId && !s.IsDeleted)
...
//                        currVendorAccountId = request.context.Account.FirstOrDefault(a => a.UserId == request.UserID).AccountId;

//                    var result = new InvoiceDTO();

//                    result.VendorBalance = request.context.Account
//                                           .Where(s => !s.IsDeleted && s.RoleId == (int)EnumRole.Vendor
//                                                    && (currVendorAccountId > 0 ? s.AccountId == currVendorAccountId : true))
//                                           .Sum(t => t.Balance);

//                    result.TreasuryBalance = request.context.Account.FirstOrDefault(s => s.AccountId == (int)EnumAccountId.Main_Treasury).Balance;

//                    var TotalDelivered = request.context.AccountTransaction
...
//                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                             .Sum(t => t.Total);

//                    var TotalRefunded = request.context.AccountTransaction
//                             .Where(t => (t.ReceiverId != (int)EnumAccountId.Main_Treasury && t.ReceiverId != (int)EnumAccountId.RED_Main_Account)
//                                       && t.StatusId == (int)EnumStatus.Refunded && t.TypeId == (int)EnumTransactionType.Withdraw)
//                             .Sum(t => t.Total);

//                    var TotalOthers = request.context.AccountTransaction
...
//                    result.REDFees = request.context.AccountTransaction
//                                     .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                 && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                     .Sum(t => t.Total);

//                    result.ShippingFees = request.context.AccountTransaction
//                                          .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                          .Sum(t => t.ShippingFees);

//                    result.PackingFees = request.context.AccountTransaction
//                                          .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                          .Sum(t => t.PackingFees);

//                    result.WeightFees = request.context.AccountTransaction
//                                          .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                          .Sum(t => t.WeightFees);

//                    result.SizeFees = request.context.AccountTransaction
//                                          .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                          .Sum(t => t.SizeFees);

//                    result.PartialDeliveryFees = request.context.AccountTransaction
//                                          .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                          .Sum(t => t.PartialDeliveryFees);

//                    result.PickupFees = request.context.AccountTransaction
//                                          .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                          .Sum(t => t.PickupFees);

//                    result.CancelFees = request.context.AccountTransaction
//                                          .Where(t => t.ReceiverId == (int)EnumAccountId.RED_Main_Account && t.TypeId == (int)EnumTransactionType.Deposit
//                                                      && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                          .Sum(t => t.CancelFees);

//                    result.Withdraw = request.context.AccountTransaction
//                                      .Where(t => t.ReceiverId != (int)EnumAccountId.RED_Main_Account && t.ReceiverId != (int)EnumAccountId.Main_Treasury
//                                               && t.TypeId == (int)EnumTransactionType.Withdraw
//                                               && (currVendorAccountId > 0 ? t.VendorId == currVendorAccountId : true))
//                                      .Sum(t => t.Total);

//                    result.CashTransferDTOs = request.context.CashTransfer
//                    .Where(s => !s.IsDeleted && (currVendorAccountId > 0 ? s.ReceiverId == currVendorAccountId : true))
//                    .Select(s => new CashTransferDTO
//                    {
...
//                        AccountTransactionDTOs = request.context.AccountTransaction
//                        .Where(t => t.ReceiverId != (int)EnumAccountId.Main_Treasury
//                                 && request.context.Shipment.Where(sh => sh.CashTransferId == s.CashTransferId).Select(sh => sh.ShipmentId).Contains(t.ShipmentId.Value)
...
//                            PickupRefId = t.PickupRequestId.HasValue ? t.PickupRequest.RefId : null,
...
//                            StatusName = t.Shipment.Status.Name,
...
//                            },
//                        })
//                    }).OrderByDescending(t => t.CashTransferId).ToList();
...
//        private static IQueryable<CashTransferDTO> ApplyFilter(IQueryable<CashTransferDTO> query, CashTransferDTO filter)
//        {
