...
//            RunBase(request, response, (PickUpRequestRequest req) =>
//            {
//                try
//                {
...
//                    if (request.PickupDTO.PickupRequestTypeId == (int)EnumPickupRequestType.StockPickup)
//                    {
//                        newPickupRequest.RefId = BaseHelper.GenerateRefId(EnumRefIdType.Stock_Pickup, newPickupRequest.PickupRequestId);
//                        newPickupRequest.PickupFees = 0; // No fees for stock pickup

//                        foreach (var item in request.PickupDTO.PickupItems)
//                        {
//                            request.context.PickupRequestItem.Add(new PickupRequestItem()
//                            {
...
//                        }
//                    }
//                    else // Save delivery pickup shipments
//                    {
//                        newPickupRequest.RefId = BaseHelper.GenerateRefId(EnumRefIdType.Delivery_Pickup, newPickupRequest.PickupRequestId);
//                        newPickupRequest.PickupFees = BaseHelper.CalcPickupFees(request.PickupDTO.PickupItems.Count());
...
//                                StatusId = (int)EnumStatus.Ready_For_Pickup
//                            });
//                            request.context.Shipment.FirstOrDefault(s => s.ShipmentId == item.ShipmentId).PickupRequestId = newPickupRequest.PickupRequestId;
//                        }
//                    }

//                    // Include return shipments with pickup
//                    if (request.PickupDTO.IncludeReturns)
//                    {
//                        var ships = request.context.Shipment.Where(s => s.StatusId == (int)EnumStatus.Ready_For_Return && s.PickupRequestId == null);
...
//                            });
//                            ship.PickupRequestId = newPickupRequest.PickupRequestId;
//                        }
//                    }
...
//                    response.Message = "New Pickup Request " + newPickupRequest.RefId + " Added";
...
//                try
//                {
//                    var model = request.context.PickupRequest.Find(request.PickupDTO.PickupRequestId);
//                    if (model != null)
//                    {
//                        model.VendorAddress = request.PickupDTO.VendorAddress;
//                        model.ZoneId = request.context.City.FirstOrDefault(c => c.Id == request.PickupDTO.AreaId).ZoneId.Value;
...
//                        response.Message = "Pickup request " + request.PickupDTO.RefId + " updated";
//                        response.Success = true;
//                        response.StatusCode = HttpStatusCode.OK;
//                    }
//                    else
...
//                try
//                {
//                    var pickup = request.context.PickupRequest.FirstOrDefault(r => r.PickupRequestId == request.PickupDTO.PickupRequestId);

//                    pickup.StatusId = (int)EnumStatus.Cancelled_Pickup;

//                    var pickItems = request.context.PickupRequestItem.Where(i => i.PickupRequestId == pickup.PickupRequestId);
//                    foreach (var item in pickItems)
//                    {
//                        if (item.StatusId == (int)EnumStatus.Ready_For_Return || item.StatusId == (int)EnumStatus.Assigned_For_Return || item.StatusId == (int)EnumStatus.Out_For_Return)
//                            item.StatusId = (int)EnumStatus.Ready_For_Return;
//                        else
//                            item.StatusId = (int)EnumStatus.Cancelled_Pickup;
//                    }

//                    var shipments = request.context.Shipment.Where(s => s.PickupRequestId == pickup.PickupRequestId);
//                    foreach (var ship in shipments)
//                    {
//                        if (ship.StatusId == (int)EnumStatus.Ready_For_Return || ship.StatusId == (int)EnumStatus.Assigned_For_Return || ship.StatusId == (int)EnumStatus.Out_For_Return)
...
//                            Title = request.context.Status.FirstOrDefault(s => s.Id == (int)EnumStatus.Cancelled_Pickup).Name,
...
//        public static PickUpRequestResponse AssignPickupRequests(PickUpRequestRequest request)
//        {
//            var response = new PickUpRequestResponse();
//            RunBase(request, response, (PickUpRequestRequest req) =>
//            {
//                try
//                {
//                    var pickupRequests = request.context.PickupRequest.Where(r => request.PickupDTO.PickupIDs.Contains(r.PickupRequestId));
//                    foreach (var pickup in pickupRequests)
//                    {
//                        pickup.DeliveryManId = request.PickupDTO.DeliveryManId;
//                        pickup.StatusId = (int)EnumStatus.Assigned_For_Pickup;
...
//                            else
//                                item.StatusId = (int)EnumStatus.Assigned_For_Pickup;
//                        }

//                        var shipments = request.context.Shipment.Where(s => s.PickupRequestId == pickup.PickupRequestId);
//                        foreach (var ship in shipments)
//                        {
//                            ship.DeliveryManId = request.PickupDTO.DeliveryManId;

//                            if (ship.StatusId == (int)EnumStatus.Ready_For_Return || ship.StatusId == (int)EnumStatus.Assigned_For_Return || ship.StatusId == (int)EnumStatus.Out_For_Return)
//                                ship.StatusId = (int)EnumStatus.Assigned_For_Return;
//                            else
//                                ship.StatusId = (int)EnumStatus.Assigned_For_Pickup;
...
//                            {
//                                FollowUpTypeId = (int)EnumFollowupType.Order_Updated,
//                                Title = request.context.Status.FirstOrDefault(s => s.Id == (int)EnumStatus.Assigned_For_Pickup).Name,
//                                Comment = request.context.CommonUser.FirstOrDefault(u => u.Id == request.PickupDTO.DeliveryManId).Name,
//                                CreatedBy = request.UserID,
//                                ShipmentId = ship.ShipmentId,
//                                StatusId = (int)EnumStatus.Assigned_For_Pickup
//                            };
//                            FollowUpService.Add(follow, request.context);
//                        }
//                    }
...
//                    response.Message = "Pickup request(s) updated";
//                    response.Success = true;
//                    response.StatusCode = HttpStatusCode.OK;
//                }
//                catch (Exception ex)
...
//        public static PickUpRequestResponse UnAssignPickupRequests(PickUpRequestRequest request)
//        {
//            var response = new PickUpRequestResponse();
//            RunBase(request, response, (PickUpRequestRequest req) =>
//            {
...
//                {
//                    var pickup = request.context.PickupRequest.FirstOrDefault(r => r.PickupRequestId == request.PickupDTO.PickupRequestId);
//                    pickup.DeliveryManId = null;
//                    pickup.StatusId = (int)EnumStatus.Ready_For_Pickup;
...
//                        ship.StatusId = (int)EnumStatus.Ready_For_Pickup;
...
//                try
//                {
//                    var pickups = request.context.PickupRequest.Where(r => request.PickupDTO.PickupIDs.Contains(r.PickupRequestId));
...
//                            pickupItems = request.context.PickupRequestItem.Where(r => request.PickupDTO.PickupItemsIDs.Contains(r.PickupRequestItemId));
//                        else
//                            pickupItems = request.context.PickupRequestItem.Where(i => i.PickupRequestId == pickup.PickupRequestId);
...
//                            if (pickupItem.ShipmentId.HasValue) // Shipment
//                            {
//                                var ship = request.context.Shipment.FirstOrDefault(r => r.ShipmentId == pickupItem.ShipmentId);
...
//                                foreach (var item in shipItems)
//                                {
//                                    if (item.StatusId == (int)EnumStatus.Picked_Up)
//                                        item.StatusId = (int)EnumStatus.At_Warehouse;
//                                    else
//                                        item.StatusId = (int)EnumStatus.Not_Received;
//                                }
...
//                            else // Stock Product
//                            {
//                                if (request.PickupDTO.StatusId == (int)EnumStatus.At_Warehouse && pickupItem.StatusId == (int)EnumStatus.At_Warehouse)
//                                {
//                                    var product = request.context.VendorProduct.FirstOrDefault(p => p.VendorProductId == pickupItem.VendorProductId);
...
//                    foreach (var pickupItem in pickupItems)
//                    {
//                        if (pickupItem.StatusId == (int)EnumStatus.Ready_For_Return || pickupItem.StatusId == (int)EnumStatus.Assigned_For_Return || pickupItem.StatusId == (int)EnumStatus.Out_For_Return)
...
//                    if (pickup.PickupRequestTypeId == (int)EnumPickupRequestType.DeliveryPickup && PickedUpCount < BaseHelper.Constants.MinFreeShipCount)
//                    {
//                        var VendorAccountId = request.context.Account.FirstOrDefault(a => a.UserId == VendorId).AccountId;
...
//            RunBase(request, res, (PickUpRequestRequest req) =>
//            {
//                try
//                {
...
//                        PickupItems = request.HasPickupItemDTO ? p.PickupRequestItem.Select(s => new PickupItemDTO
//                        {
...
//                            RefId = s.ShipmentId.HasValue ? s.Shipment.RefId : null,
//                            Quantity = s.Quantity,
//                            Price = s.Price,
...
//                try
//                {
//                    var query = request.context.PickupRequestItem.Where(p => p.PickupRequestId == request.PickupItemDTO.PickupRequestId)
//                    .Select(p => new PickupItemDTO
//                    {
//                        PickupRequestItemId = p.PickupRequestItemId,
//                        PickupRequestId = p.PickupRequestId,
//                        VendorProductName = p.ShipmentId.HasValue ? p.Shipment.RefId : p.VendorProduct.Name,
//                        Quantity = p.Quantity,
//                        Price = p.Price,
//                        IsDeleted = p.IsDeleted,
//                        StatusId = p.StatusId,
...
//        private static IQueryable<PickupDTO> ApplyFilter(IQueryable<PickupDTO> query, PickupDTO filter)
//        {
//            if (!string.IsNullOrEmpty(filter.Search))
//                query = query.Where(c => c.RefId.Contains(filter.Search)
