...
namespace LegendCR.BL.Services
{
    public class UserService : BaseService
    {
        public static UserResponse GetAllUsers(UserRequest request)
        {
            ...
            RunBase(request, res, (UserRequest req) =>
            {
                try
                {
                    var query = request.context.CommonUser.Where(c => !c.IsDeleted).Select(c => new UserDTO
                    {
                        Id = c.Id,
                        Name = c.Name,
                        ...
                        ZoneName = c.ZoneId.HasValue ? request.context.Zone.FirstOrDefault(u => u.Id == c.ZoneId).NameAr : "",
                        AreaName = c.AreaId.HasValue ? request.context.City.FirstOrDefault(a => a.Id == c.AreaId).CityNameAr : "",

                        //CourierShipments = request.HasCourierShipments ? c.ShipmentDeliveryMan
                        //.Where(r => r.StatusId == (int)EnumStatus.Assigned_For_Delivery
                        ...
                        //    RefId = s.RefId,
                        //    CustomerDetailsDTO = new CustomerDetailsDTO { CustomerName = s.CustomerName },
                        //    VendorDetailsDTO = new VendorDetailsDTO { VendorName = s.VendorName },
                        //    StatusDTO = new StatusDTO { Id = s.StatusId, Name = s.Status.Name }
                        //}) : null,
                    });
        ...
        public static UserResponse GetUser(UserRequest request)
        {
            ...
            RunBase(request, res, (UserRequest req) =>
            {
                try
                {
                    var query = request.context.CommonUser.Where(c => !c.IsDeleted && c.Id == request.UserDTO.Id).Select(c => new UserDTO
                    {
                        Id = c.Id,
                        Name = c.Name,
                        NationalId = c.NationalId,
        ...
        public static UserResponse Login(UserRequest request)
        {
            ...
            RunBase(request, res, (UserRequest req) =>
            {
                try
                {
                    ...
                    var user = request.context.CommonUser.Where(c => !c.IsDeleted && c.Email == model.Email && c.Password == model.Password)
                          .Select(c => new { user = c, role = c.Role }).FirstOrDefault();
        ...
        public static UserResponse DeleteUser(UserRequest request)
        {
            ...
            RunBase(request, res, (UserRequest req) =>
            {
                ...
                {
                    var model = request.UserDTO;
                    var user = request.context.CommonUser.FirstOrDefault(c => !c.IsDeleted && c.Id == model.Id);
                    if (user != null)
                    {
            ...
            RunBase(request, res, (UserRequest req) =>
            {
                try
                {
                    ...
                    if (user != null)
                    {
                        ...
                        var UserExist = request.context.CommonUser.Any(m => m.Email == request.UserDTO.Email && m.Email != user.Email && m.IsDeleted == false);
                        if (!UserExist)
                        {
        ...
        public static UserResponse AddUser(UserRequest request)
        {
            ...
            RunBase(request, res, (UserRequest req) =>
            {
                try
                {
                    var UserExist = request.context.CommonUser.Any(m => m.Email == request.UserDTO.Email && m.IsDeleted == false);
                    if (!UserExist)
                    {
                        ...
                        if (User.RoleId == (int)EnumRole.Vendor || User.RoleId == (int)EnumRole.DeliveryMan)
                        {
        ...
        private static IQueryable<UserDTO> ApplyFilter(IQueryable<UserDTO> query, UserDTO filter)
        {
            if (!string.IsNullOrEmpty(filter.Search))
                query = query.Where
                    (c => (c.Name.Contains(filter.Search))
            ...
            if (filter.RoleID == (int)EnumRole.Employee)
                query = query.Where(u => u.RoleID != (int)EnumRole.Vendor && u.RoleID != (int)EnumRole.Customer && !u.IsDeleted);

            if (filter.RoleID > 0 && filter.RoleID != (int)EnumRole.Vendor && filter.RoleID != (int)EnumRole.Employee)
                query = query.Where(c => c.RoleID == filter.RoleID);

            if (filter.StaffOnly)
                query = query.Where(c => c.RoleID != (int)EnumRole.Vendor && c.RoleID != (int)EnumRole.Customer);
            ...
            if (!string.IsNullOrWhiteSpace(filter.PhoneNumber))
                query = query.Where(c => c.PhoneNumber.Trim().IndexOf(filter.PhoneNumber.Trim(), StringComparison.OrdinalIgnoreCase) >= 0);

            if (filter.HasCourierShipments)
                query = query.Where(c => c.CourierShipments != null && c.CourierShipments.Count() > 0);
